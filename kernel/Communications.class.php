 ...
 * @author: Luis Pater
 * @date: 2009-07-29
 * $Id: Communications.class.php 788 2011-11-01 10:31:58Z luis $
 */
class Communications {
    ...
    private function httpRequest($str_url, $array_request, $array_settings, $bln_is_post, $bln_need_header = true) {
        ...
        if ($str_ip = $this->getIpByHostName($array_parsed_url["host"])) {
            ...
            curl_setopt($obj_ch, CURLOPT_HTTPHEADER, array("Host:".$array_parsed_url["host"]));
        }
        else {
            // 未使用缓存机制
            curl_setopt($obj_ch, CURLOPT_URL, $str_url);
    ...
    public function httpPostRequest($str_url, $array_request = array(), $array_settings = array(), $bln_need_header = true) {
        return $this->httpRequest($str_url, $array_request, $array_settings, true, $bln_need_header);
    }

    public function httpGetRequest($str_url, $array_request = array(), $array_settings = array(), $bln_need_header = true) {
